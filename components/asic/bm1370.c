...
static bool do_frequency_transition(float target_frequency) {
    ...
    if (fmod(current, step) != 0) {
        ...
        BM1370_send_hash_frequency(current);
        vTaskDelay(100 / portTICK_PERIOD_MS);
    }
    ...
    BM1370_send_hash_frequency(target);
    return true;
}
...
static int count_asic_chips(void) {
    _send_BM1370(TYPE_CMD | GROUP_ALL | CMD_READ, (uint8_t[]){0x00, 0x00}, 2, false);
...
uint8_t BM1370_init(uint64_t frequency, uint16_t asic_count)
{
    ...
    for (int i = 0; i < 4; i++) {
        _send_BM1370(TYPE_CMD | GROUP_ALL | CMD_WRITE, init_cmd, 6, false);
    }
    ...
    for (int i = 0; i < sizeof(init_cmds) / sizeof(init_cmds[0]); i++) {
        _send_BM1370(TYPE_CMD | GROUP_ALL | CMD_WRITE, init_cmds[i], 6, false);
    }
    ...
    for (int i = 0; i < chip_counter; i++) {
        ...
        for (int j = 0; j < sizeof(chip_init_cmds) / sizeof(chip_init_cmds[0]); j++) {
            _send_BM1370(TYPE_CMD | GROUP_SINGLE | CMD_WRITE, chip_init_cmds[j], 6, false);
        }
    ...
    _send_BM1370(TYPE_CMD | GROUP_ALL | CMD_WRITE, (uint8_t[]){0x00, 0x10, 0x00, 0x00, 0x1E, 0xB5}, 6, false);
    _send_BM1370(TYPE_CMD | GROUP_ALL | CMD_WRITE, (uint8_t[]){0x00, 0xA4, 0x90, 0x00, 0xFF, 0xFF}, 6, false);
