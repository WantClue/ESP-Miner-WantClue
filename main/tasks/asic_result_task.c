...
void ASIC_result_task(void *pvParameters)
{
    ...
    while (1)
    {
       ...
       // ESP_LOGI(TAG, "AsicNr: %d Ver: %08" PRIX32 " Nonce %08" PRIX32 " diff %.1f of %ld.", asic_result->asic_nr,asic_result->rolled_version, asic_result->nonce, nonce_diff, pool_difficulty);
        ESP_LOGI(TAG, "AsicNr: %d Ver: %08" PRIX32 " Nonce %08" PRIX32 " diff %.1f of %ld.", asic_result->asic_nr,asic_result->rolled_version, asic_result->nonce, nonce_diff, GLOBAL_STATE->ASIC_TASK_MODULE.active_jobs[job_id]->pool_diff);
